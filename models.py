...
class HasCommitteeMember(StructuredRel, Model):
    ...
    def get_model_info(self) -> dict:
        return {"unique_id": self.unique_id, "position": self.position}


class Person(StructuredNode, Model):
    unique_id = (
        UniqueIdProperty()
    )  # TODO: Replace this with custom property that uses ULID
...
class Society(StructuredNode, Model):
    ...
    # Relationships
    committee_members = RelationshipTo(
        "Person", RELATIONSHIPS.HAS_COMMITTEE_MEMBER, model=HasCommitteeMember
    )
    embedding = ArrayProperty(base_property=FloatProperty(default=None))
    ...
    def get_model_info(self) -> dict:
        return {"unique_id": self.unique_id, "name": self.name}
